...
if [ -f "src/_data/profiles.js" ]; then
    ...
    # Show default pen name (first profile) and env var if set
    DEFAULT_PEN_NAME=$(node -e "const p = require('./src/_data/profiles.js'); console.log(Object.keys(p.profiles)[0])")
    if [ -n "$ELEVENTY_PEN_NAME" ]; then
        echo -e "${BLUE}  Active Pen Name (env):${NC} ${ELEVENTY_PEN_NAME}"
    else
        echo -e "${BLUE}  Default Pen Name:${NC} ${DEFAULT_PEN_NAME}"
    fi
...
echo "  • Active pen name controlled by: ELEVENTY_PEN_NAME env var (defaults to first profile)"
echo "  • build-all-sites.js sets ELEVENTY_PEN_NAME for each build iteration"
