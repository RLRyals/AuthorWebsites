...
for (const [penName, profile] of Object.entries(profiles)) {
  ...
  const missing = ['outputFolder', 'blogFolder', 'styles'].filter(f => !profile[f]);
  if (missing.length > 0) {
    logError(`Skipping ${penName}: missing required fields: ${missing.join(', ')}`);
    buildResults.push({ penName, success: false, duration: '0', error: `Missing fields: ${missing.join(', ')}` });
    continue;
  }
  ...
  try {
    ...
    // Run the build with pen name passed via environment variable
    // This avoids modifying profiles.js on disk
    execSync('npx @11ty/eleventy', {
      env: { ...process.env, ELEVENTY_PEN_NAME: penName },
      stdio: 'inherit',
